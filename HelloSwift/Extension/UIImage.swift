...
extension UIImage {
    ...
    /// <#Description#>
    /// - Parameters:
    ///   - fontCode: <#fontCode description#>
    ///   - color: 图片颜色
    ///   - size: 图片大小
    class func imageWithIcon(fontCode: IconFont, color: UIColor, size: CGFloat) -> UIImage {
        let realSize = size * UIScreen.main.scale
        UIGraphicsBeginImageContext(CGSize(width: realSize, height: realSize))
        let font = UIFont(name: font_iconFont, size: realSize)
        let text:NSString = NSString(string: fontCode.rawValue)
        
        print(fontCode.rawValue)
        
        text.draw(at: CGPoint(x: 0, y: 0), withAttributes: [NSAttributedString.Key.font:font ?? UIFont(),
                                                     NSAttributedString.Key.foregroundColor : color.cgColor])

        let image = UIImage(cgImage: (UIGraphicsGetImageFromCurrentImageContext()?.cgImage!)!, scale: UIScreen.main.scale, orientation: .up)
