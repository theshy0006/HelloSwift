...
extension UIViewController {
    ...
    // 设置导航栏左边图标字体按钮
    func setNavBarLeftBtn(fontCode: IconFont, _ fontColor: UIColor = color_title,
                          _ fontSize: CGFloat = 24.0, selector:Selector) {
        let button = UIButton(type: .custom)
        button.titleLabel?.font = UIFont(name: font_iconFont, size: fontSize)
        button.setTitle(fontCode.rawValue, for: .normal)
        button.setTitleColor(fontColor, for: .normal)
        self.navigationItem.leftBarButtonItem = UIBarButtonItem(customView: button)
    }

    // 设置导航栏右侧图标字体按钮
    func setNavBarRightBtn(fontCode: IconFont, _ fontColor: UIColor = color_title,
                           _ fontSize: CGFloat = 24.0, selector:Selector) {
        ...
        let spacer = UIBarButtonItem(barButtonSystemItem: .fixedSpace, target: nil,
                                     action: nil)
        spacer.width = 10
        self.navigationItem.rightBarButtonItems = [rightButtonItem, spacer]
    }
